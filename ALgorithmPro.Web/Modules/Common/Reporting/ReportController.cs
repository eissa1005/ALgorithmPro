...
namespace ALgorithmPro
{
    ...
    public class ReportController : Controller
    {
        ...
        public ActionResult GetReport(string key, string opt, string ext)
        {
            return Execute(key, opt, ext, download: true, printing: true);
        }
        ...
        private byte[] RenderAsPdf(IReport report, string key, string opt)
        {
            ...
            renderUrl += "&print=1";
            string PDFPath = string.Format(@"{0}\{1}", Environment.CurrentDirectory, "wkhtmltopdf.exe");
            var converter = new HtmlToPdfConverter();
            var wkhtmlPath = Path.Combine(typeof(ReportController).Assembly.Location, PDFPath);
            if (!System.IO.File.Exists(wkhtmlPath)) // linux?
                wkhtmlPath = Path.Combine(typeof(ReportController).Assembly.Location, "wkhtmltopdf");
