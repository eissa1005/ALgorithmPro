...
using MyRepository = ALgorithmPro.ALgorithm.Repositories.TransferInRepository;
...
{
    [Route("Services/ALgorithm/TransferIn/[action]")]
    [ConnectionKey(typeof(MyRow)), ServiceAuthorize(typeof(MyRow))]
    public class TransferInController : ServiceEndpoint
    {
        ...
        {
            string SQL = "SELECT ISNULL(MAX(TR_NO),0) AS MAXNO FROM ASTRH WHERE Status = 1 AND TR_TY=" + (int)TRTYType.TransferIN + " AND StoreID=" + request.StoreID + "";
        ...
        public FileContentResult ListExcel(IDbConnection connection, ListRequest request)
        {
            ...
            var report = new DynamicDataReport(data, request.IncludeColumns, typeof(Columns.TransferInColumns), HttpContext.RequestServices);
