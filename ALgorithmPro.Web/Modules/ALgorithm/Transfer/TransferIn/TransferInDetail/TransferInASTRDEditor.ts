...
namespace ALgorithmPro.ALgorithm {
    ...
    import FLD = ALgorithmPro.ALgorithm.TransferInASTRDRow.Fields;

    @Serenity.Decorators.registerClass()
    export class TransferInASTRDEditor extends AS.GridBaseEditor<AddInventoryASTRDRow> {
        protected getColumnsKey() { return 'ALgorithm.TransferInASTRD'; }
        protected getDialogType() { return TransferInASTRDDialog; }
        ...
        constructor(container: JQuery) {
            ...
            var PrefixId = BS.GetPrefixId(TransferInForm.formKey);
            this.form = new TransferInForm(PrefixId);

            // KeyDown 👈
            this.slickGrid.onKeyDown.subscribe((e) => {
                if (e.keyCode == AS.KeyCode.F2) {

                    TransferInASTRDEditor.GridName = this.slickGrid;
                    SelectTRTY = AS.TRTYType.TransferIN;
        ...
        protected getItemCssClass(item: ALgorithm.TransferInASTRDRow, index: number): string {
        ...
        private UpdateFooter() {
            ...
            if (!AS.IsNullObject(ASTRD)) {
                if (RowCount > 0) {
                    ...
                    this.form.Total.value = Q.round(NET, 0);
