...
using MyRepository = ALgorithmPro.ALgorithm.Repositories.StoresRepository;
using MyRow = ALgorithmPro.ALgorithm.Entities.StoresRow;
...
{
    [Route("Services/ALgorithm/Stores/[action]")]
    ...
    public class StoresController : ServiceEndpoint
    {
        ...
        {
            string SQL = "SELECT  ISNULL(MAX(StoreID),0) AS MAXNO FROM Stores WHERE Status = 1";
            var Query = connection.Query<string>(SQL);
            var MaxNO = Query.ToList().First();
            return GetNextNumberHelper.GetNextNumber(connection, request, MyRow.Fields.StoreID, MaxNO);
        }
