...
using MyRow = ALgorithmPro.ALgorithm.Entities.CashPayedRow;

namespace ALgorithmPro.ALgorithm.Repositories
{
    public class CashPayedRepository : BaseRepository
    {
        ...
        public CashPayedRepository(IRequestContext context) : base(context)
        {
            SelectTRTY = TRTYType.CashPayed;
        }
        ...
        public SaveResponse Create(IUnitOfWork uow, SaveRequest<MyRow> request)
        {
            ...
            try
            {
                ...
                header.TR_TY = (int)TRTYType.CashPayed;
                header.TRTY_NAME = "يومية الخزينة - سند صرف";
                header.TR_DS = (int)TRDSTYPE.CashPayed;
                ...
                header.HDSCR_AR = "إيصال سداد نقدية : " + header.ACC_NAME;
                header.HDSCR_AR = "إيصال سداد نقدية : " + header.ACC_NAME;
        ...
        {
            if (SelectTRTY == TRTYType.CashPayed)
            {
                try
                {
        ...
        public ListResponse<MyRow> List(IDbConnection connection, ListRequest request)
        {
            ...
            try
            {
                if (SelectTRTY == TRTYType.CashPayed)
                {
                    lst.Entities = connection.List<MyRow>(s => s.SelectTableFields()).Where<MyRow>(x => x.TR_TY == (int)TRTYType.CashPayed).ToList();
                    return lst;
                }
                else
                {
        ...
        public static void CashDetail(IDbConnection connection, CashPayedRow ACCTRH)
        {
            if (SelectTRTY == TRTYType.CashPayed)
            {
        ...
        public static void UpdateCashDetail(IDbConnection connection, CashPayedRow ACCTRH)
        {
            if (SelectTRTY == TRTYType.CashPayed)
            {
