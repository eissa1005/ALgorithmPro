...
using MyRepository = ALgorithmPro.ALgorithm.Repositories.ExpensesRepository;
...
{
    [Route("Services/ALgorithm/Cash/Expenses/[action]")]
    ...
    public class ExpensesController : ServiceEndpoint
    {
        ...
        {
            string SQL = "SELECT ISNULL(MAX(TR_NO),0) AS MAXNO FROM ASACCTRH WHERE Status = 1 AND TR_TY=" + (int)TRTYType.Expenses + " AND StoreID=" + request.StoreID + "";
        ...
        public FileContentResult ListExcel(IDbConnection connection, ListRequest request)
        {
            ...
            var report = new DynamicDataReport(data, request.IncludeColumns, typeof(Columns.ExpensesColumns), HttpContext.RequestServices);
