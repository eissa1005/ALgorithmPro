...
using MyRow = ALgorithmPro.ALgorithm.Entities.CashReturnASTRHRow;
using static ALgorithmPro.Contstants;

namespace ALgorithmPro.ALgorithm.Repositories
{
    public class CashReturnASTRHRepository : BaseRepository
    {
        private static MyRow.RowFields Fld => MyRow.Fields;
        public static TRTYType SelectTRTY { set; get; }
        public static List<CashReturnASTRDRow> listASTRD { set; get; }
        public CashReturnASTRHRepository(IRequestContext context) : base(context)
        {
            SelectTRTY = TRTYType.CashReturn;
        }
        public SaveResponse Create(IUnitOfWork uow, SaveRequest<MyRow> request)
        {
            ...
            try
            {
                ...
                int LNNO = 1;
                if (SelectTRTY == TRTYType.CashReturn)
                {
                    listASTRD.ForEach(s =>
                    {
                        s.DetailID = request.Entity.HeaderID;
                        s.TR_NO = request.Entity.TR_NO;
                        s.TR_TY = (int)TRTYType.CashReturn;
                        s.TRTY_NAME = request.Entity.TRTY_NAME;
                        s.TR_DS = (int)TRDSTYPE.CashReturn;
                        s.GRP = (int)GRPTYPE.CashReturn;
        ...
        public ListResponse<MyRow> List(IDbConnection connection, ListRequest request)
        {
            ...
            try
            {
                if (SelectTRTY == TRTYType.CashReturn)
                {
                    lst.Entities = connection.List<MyRow>(s => s.SelectTableFields()).Where<MyRow>(x => x.TR_TY == (int)TRTYType.CashReturn).ToList();
                    return lst;
                }
                else
                {
