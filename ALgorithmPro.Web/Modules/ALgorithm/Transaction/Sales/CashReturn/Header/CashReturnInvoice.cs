...
{
    public partial class CashReturnInvoice
    {
        public static double ProHeaderID { set; get; }
        public static int? PRNT_CNT { set; get; }
        public CashReturnInvoice()
        {
            InitializeComponent();
        }
        public CashReturnInvoice(double HeaderID)
        {
            InitializeComponent();
        ...
        public void Data(double HeaderID)
        {
            using (var serviceScope = ServiceActivator.GetScope())
            {
                ...
                string SQL = "SELECT * FROM ASTRDVIEW WHERE HeaderID=" +HeaderID+ " AND TR_TY="+(int)TRTYType.CashReturn+"";
            ...
            try
            {
                using (var serviceScope = ServiceActivator.GetScope())
                {
                    ...
                    using (var connection = ((ISqlConnections)sqlConnections).NewByKey("Default"))
                    {
                        ...
                        string SQL = "SELECT * FROM ASTRDVIEW WHERE HeaderID=" + ProHeaderID + " AND TR_TY=" + (int)TRTYType.CashReturn + "";
                        var ASTRH = untwork.Connection.Query<CashReturnRow>(SQL).FirstOrDefault();
                        if (ASTRH != null && ASTRH.PRT_CNT == 0)
                        {
                            PRNT_CNT = 0;
                            ASTRH.PRT_CNT += 1;
                            untwork.Connection.UpdateById<CashReturnRow>(ASTRH, ExpectedRows.ZeroOrOne);
                            xrIMGCNT.Text = "«’· ";
                        }
