...
/// <reference path="../../../../../../Imports/ServerTypings/ALgorithm.PackageRow.ts" />

namespace ALgorithmPro.ALgorithm {

    import FLD = ALgorithmPro.ALgorithm.CashReturnASTRDRow.Fields;

    @Serenity.Decorators.registerClass()
    export class CashReturnASTRDEditor extends AS.GridBaseEditor<CashReturnASTRDRow> {
        protected getColumnsKey() { return 'ALgorithm.CashReturnASTRD'; }
        protected getDialogType() { return CashReturnASTRDDialog; }
        ...
        constructor(container: JQuery) {
            ...
            var PrefixId = BS.GetPrefixId(CashReturnForm.formKey);
            this.form = new CashReturnForm(PrefixId);
            ...
            this.slickGrid.onKeyDown.subscribe((e) => {
                ...
                if (e.keyCode == AS.KeyCode.F2) {
                    var dialog;
                    SelectTRTY = AS.TRTYType.CashReturn;
                    CashReturnASTRDEditor.GridName = this.slickGrid;
        ...
        // validateEntity 👈
        protected validateEntity(row: CashReturnASTRDRow) {
            ...
            if (!AS.IsNullValue(this.form.ReferenceNo)) {
                ...
                ASTRD = Q.tryFirst(CashSalesASTRDRow.getLookup().items,
                    x => x.TR_TY == AS.TRTYType.CashSales && x.ACC_NO2 == ACCNO && x.TR_NO == ReferenceNumber)
            }

            let RATE = BS.GetCurrencyRAT(CashReturnDialog.OldValue, CashReturnDialog.NewValue);
        ...
        //PackageChange
        private PackageChange(Field, index, items: CashReturnASTRDRow) {
        ...
        // UpdateGrid 
        private UpdateGrid(index, row: CashReturnASTRDRow) {

            if (AS.IsNullObject(row)) return;
            var price = row.Price;
            var ReturnQty = AS.IsNull(row.ReturnQty);
            var value = ReturnQty * price;
