...
using MyRepository = ALgorithmPro.ALgorithm.Repositories.SalesRepository;
...
{
    [Route("Services/ALgorithm/Sales/[action]")]
    ...
    public class SalesController : ServiceEndpoint
    {
        ...
        {
            string SQL = "SELECT ISNULL(MAX(TR_NO),0) AS MAXNO FROM ASTRH WHERE Status = 1 AND TR_TY="+(int)TRTYType.Sales+" AND StoreID=" + request.StoreID+"";
