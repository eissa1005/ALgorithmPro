...
namespace ALgorithmPro.ALgorithm {
    ...
    export class ReturnASTRDEditor extends AS.GridBaseEditor<ReturnASTRDRow> {
        ...
        constructor(container: JQuery) {
            ...
            var PrefixId = BS.GetPrefixId(ReturnSalesForm.formKey);
            this.form = new ReturnSalesForm(PrefixId);
            ...
            this.slickGrid.onKeyDown.subscribe((e) => {
                ...
                if (e.keyCode == AS.KeyCode.F2) {
                    var dialog;
                    SelectTRTY = AS.TRTYType.ReturnSales;

                    ReturnASTRDEditor.GridName = this.slickGrid;
        ...
        // validateEntity 👈
        protected validateEntity(row: ReturnASTRDRow) {

            if (AS.IsNullObject(row)) return true;
            var ASTRD: SalesASTRDRow;
            ...
            if (!AS.IsNullValue(this.form.ReferenceNo)) {
                ...
                ASTRD = Q.tryFirst(SalesASTRDRow.getLookup().items,
                    x => x.TR_TY == AS.TRTYType.Sales && x.ACC_NO2 == ACCNO && x.TR_NO == ReferenceNumber)
            }

            let RATE = BS.GetCurrencyRAT(ReturnSalesDialog.OldValue, ReturnSalesDialog.NewValue);
        ...
        //PackageChange
        private PackageChange(Field, index, items: ReturnASTRDRow) {
        ...
        // UpdateGrid 
        private UpdateGrid(index, row: ReturnASTRDRow) {
