...
namespace ALgorithmPro.ALgorithm {
    ...
    export class RestoreASTRDEditor extends AS.GridBaseEditor<RestoreASTRDRow> {
        ...
        constructor(container: JQuery) {
            ...
            var PrefixId = BS.GetPrefixId(RestorePurchaseForm.formKey);
            this.form = new RestorePurchaseForm(PrefixId);
        ...
        // validateEntity 👈
        protected validateEntity(row: RestoreASTRDRow) {

            if (AS.IsNullObject(row)) return true;
            var ASTRD: PurchASTRDRow;
            if (!AS.IsNullValue(this.form.ReferenceNo)) {
                var ACCNO = this.form.ACC_NO.value;
                var ReferenceNumber = this.form.ReferenceNo.value;
                ASTRD = Q.tryFirst(PurchASTRDRow.getLookup().items,
                    x => x.TR_TY == AS.TRTYType.Purchase && x.ACC_NO2 == ACCNO && x.TR_NO == ReferenceNumber)
            }

            let RATE = BS.GetCurrencyRAT(RestorePurchaseDialog.OldValue, RestorePurchaseDialog.NewValue);
        ...
        //PackageChange
        private PackageChange(Field, index, items: RestoreASTRDRow) {
        ...
        // UpdateGrid 
        private UpdateGrid(index, row: RestoreASTRDRow) {
