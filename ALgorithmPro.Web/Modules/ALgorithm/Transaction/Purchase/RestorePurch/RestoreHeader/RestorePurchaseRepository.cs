...
using MyRow = ALgorithmPro.ALgorithm.Entities.RestorePurchaseRow;

namespace ALgorithmPro.ALgorithm.Repositories
{
    public class RestorePurchaseRepository : BaseRepository
    {
        private static MyRow.RowFields Fld => MyRow.Fields;
        public static TRTYType SelectTRTY { set; get; }
        public static IDbTransaction transaction;
        public static List<RestoreASTRDRow> listASTRD { set; get; }
        public RestorePurchaseRepository(IRequestContext context) : base(context)
        {
            SelectTRTY = TRTYType.RestorePurch;
        }
        public SaveResponse Create(IUnitOfWork uow, SaveRequest<MyRow> request)
        {
            ...
            try
            {
                ...
                int LNNO = 1;
                if (SelectTRTY == TRTYType.RestorePurch)
                {
                    listASTRD.ForEach(s =>
                    {
                        s.DetailID = header.HeaderID;
                        s.TR_NO = header.TR_NO;
                        s.TR_TY = (int)TRTYType.RestorePurch;
                        s.TRTY_NAME = header.TRTY_NAME;
                        s.TR_DT = header.TR_DT != null ? header.TR_DT : DateTime.Now;
                        s.TR_DS = (int)TRDSTYPE.RestorePurch;
                        s.GRP = (int)GRPTYPE.RestorePurch;
                    ...
                    header.TR_DS = (int)TRDSTYPE.RestorePurch;
                    header.ACC_NO = ACCNO;
        ...
        public ListResponse<MyRow> List(IDbConnection connection, ListRequest request)
        {
            ...
            try
            {
                if (SelectTRTY == TRTYType.RestorePurch)
                {
                    lst.Entities = connection.List<MyRow>(s => s.SelectTableFields()).Where<MyRow>(x => x.TR_TY == (int)TRTYType.RestorePurch).ToList();
                    return lst;
                }
                else
                {
        ...
        public static void ADDACCTRH(IDbConnection connection, RestorePurchaseRow ASTRH)
        {
            if (SelectTRTY == TRTYType.RestorePurch)
            {
        ...
        public static void ADDACCTRD(IDbConnection connection, ACCTRHRow ACCTRH, RestorePurchaseRow ASTRH)
        {
            if (SelectTRTY == TRTYType.RestorePurch)
            {
        ...
        public static void CashHeader(IDbConnection connection, ACCTRHRow ASTRH)
        {
            if (SelectTRTY == TRTYType.RestorePurch)
            {
                int CashPayed = (int)TRTYType.CashPayed;
                if (ASTRH == null || ASTRH.HeaderID == 0) return;
                transaction = connection.GetCurrentActualTransaction();
                var Header = connection.List<ACCTRHRow>().FirstOrDefault(x => x.HeaderID == ASTRH.HeaderID && x.TR_TY == CashPayed && x.StoreID == ASTRH.StoreID);
                string TRTYName = AS.GetName(connection, FunctionName.GetTRTYNAME, CashPayed.ToString());
                var MAXNO = AS.GetMaxACCTRH(connection, ASTRH.StoreID, CashPayed);
                ...
                if (ASTRH.Paid > 0)
                {
                    if (Header == null || Header.HeaderID == 0)
                    {
                        ...
                        ACCTRH.CST_CD = ASTRH.CST_CD;
                        ACCTRH.CST_NAME = ASTRH.CST_NAME;
                        ACCTRH.HDSCR_AR = " إيصـال سداد جزء من فاتورة رقم " + ASTRH.TR_NO;
                        ACCTRH.HDSCR_EN = " إيصـال سداد جزء من فاتورة رقم " + ASTRH.TR_NO;
                        ACCTRH.CurrencyID = ASTRH.CurrencyID;
                        ACCTRH.Currency_NAME = ASTRH.Currency_NAME;
                        ...
                        {
                            connection.Insert<ACCTRHRow>(ACCTRH);
                            var Detail = connection.List<ACCTRHRow>().FirstOrDefault(x => x.ASTRHID == ACCTRH.ASTRHID && x.TR_TY == CashPayed);
