...
namespace ALgorithmPro.Reports
{
    public partial class PurchInvoice
    {
        ...
        public PurchInvoice(double HeaderID)
        {
            InitializeComponent();
        ...
        public void Data(double HeaderID)
        {
            using (var serviceScope = ServiceActivator.GetScope())
            {
                ...
                string SQL = "SELECT * FROM ASTRDVIEW WHERE HeaderID=" + HeaderID + "AND TR_TY=" + (int)TRTYType.Purchase+"";
            ...
            try
            {
                using (var serviceScope = ServiceActivator.GetScope())
                {
                    ...
                    using (var connection = ((ISqlConnections)sqlConnections).NewByKey("Default"))
                    {
                        ...
                        string SQL = "SELECT * FROM ASTRDVIEW WHERE HeaderID=" + ProHeaderID + "AND TR_TY=" + (int)TRTYType.Purchase + "";
                        var ASTRH = untwork.Connection.Query<CashPurchRow>(SQL).FirstOrDefault();
