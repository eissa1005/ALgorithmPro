...
using MyRow = ALgorithmPro.ALgorithm.Entities.CashPurchRow;

namespace ALgorithmPro.ALgorithm.Repositories
{
    public class CashPurchRepository : BaseRepository
    {
        private static MyRow.RowFields Fld => MyRow.Fields;
        public static TRTYType SelectTRTY { set; get; }
        public static IDbTransaction transaction;
        public static List<CashPurchASTRDRow> listASTRD { set; get; }
        public CashPurchRepository(IRequestContext context) : base(context)
        {
            SelectTRTY = TRTYType.CashPurchase;
        ...
        public SaveResponse Create(IUnitOfWork uow, SaveRequest<MyRow> request)
        {
            ...
            try
            {
                ...
                int LNNO = 1;
                if (SelectTRTY == TRTYType.CashPurchase)
                {
                    listASTRD.ForEach(s =>
                    {
                        s.DetailID = header.HeaderID;
                        s.TR_NO = header.TR_NO;
                        s.TR_TY = (int)TRTYType.CashPurchase;
                        s.TRTY_NAME = header.TRTY_NAME;
                        s.TR_DT = header.TR_DT != null ? header.TR_DT : DateTime.Now;
                        s.TR_DS = (int)TRDSTYPE.CashPurchase;
                        s.GRP = (int)GRPTYPE.CashPurchase;
        ...
        public ListResponse<MyRow> List(IDbConnection connection, ListRequest request)
        {
            ...
            try
            {
                if (SelectTRTY == TRTYType.CashPurchase)
                {
                    lst.Entities = connection.List<MyRow>(s => s.SelectTableFields()).Where<MyRow>(x => x.TR_TY == (int)TRTYType.CashPurchase).ToList();
                    return lst;
                }
                else
                {
