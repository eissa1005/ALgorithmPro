/// <reference path="./../../ALgorithmsys/AS/AS.d.ts" />
namespace AS {
    ...
    export class GridBase<TItem, TOptions> extends Serenity.EntityGrid<TItem, TOptions> {
        ...
        protected getASGridOptions(): ExtGridOptions { return Q.deepClone(AS.DefaultMainGridOptions); }
        ...
        protected markupReady(): void {
            ...
            setTimeout(() => {
                if (this.isAutosized == false) {
                    if (this.getASGridOptions().AutoColumnSize == true) {
        ...
        protected getReportRequest(): ListReportRequest {
            ...
            var request = Q.deepClone(view ? view.params : {}) //as AS.ReportRequest;
                ...
                for (let quickFilter of quickFilters) {
                    ...
                    if (filterValue && filterValue.length > 0) {
                        ...
                        if (!Q.isEmptyOrNull(dateFrom))
                            filterText = Q.format(AS.text('Controls.FromDate', 'From {0}'), dateFrom) + ' ';

                        if (!Q.isEmptyOrNull(dateTo))
                            filterText = filterText + Q.format(AS.text('Controls.ToDate', 'To {0}'), dateTo);

                        if (!Q.isEmptyOrNull(filterText)) {
                            request.EqualityFilterWithTextValue[quickFilter.title] = filterText
                        }
                        else if (this.getASGridOptions().ShowAnyInEqualityFilterWithTextValue == true) {
                            request.EqualityFilterWithTextValue[quickFilter.title] = AS.text('Controls.All', 'all')
                        }
                    } else if (this.getASGridOptions().ShowAnyInEqualityFilterWithTextValue == true) {
                        request.EqualityFilterWithTextValue[quickFilter.title] = AS.text('Controls.All', 'all')
                    }
        ...
        protected getColumns(): Slick.Column[] {
            ...
            let extOptions = this.getASGridOptions();
            if (isEditable == true) {
            ...
            columns.forEach(column => {
                ...
                if (column.sourceItem) {
                    ...
                    if (isEditable == true && column.sourceItem.readOnly != true) {
                        if (AS.useSerenityInlineEditors) {
            ...
            if (extOptions.ShowInlineActionsColumn == true) {
                ...
                if (extOptions.ShowEditInlineButtun == true) {
                    ...
                    var title = this.isReadOnly ? AS.text('Controls.View', 'View Details') : AS.text('Controls.Edit', 'Edit');
                ...
                if (extOptions.ShowDeleteInlineButtun == true) {
                    ...
                    inlineActionsColumnContent += `<a class="inline-actions delete-row" title="${AS.text('Controls.Delete', 'Delete')}" style="padding-left: 5px;"><i class="delete-row fa fa-trash-o text-red"></i></a>`;
                }

                if (extOptions.ShowPrintInlineButtun == true) {
                    inlineActionsColumnWidth += 25;
                    inlineActionsColumnContent += `<a class="inline-actions print-row" title="${AS.text('Controls.Print', 'Print')}" style="padding-left: 5px;"><i class="print-row fa fa-print"></i></a>`;
                }
        ...
        protected getViewOptions() {
            ...
            opt.rowsPerPage = AS.DefaultMainGridOptions.RowsPerPage;
        ...
        protected onClick(e: JQueryEventObject, row: number, cell: number) {
            ...
            var items = Q.deepClone(data);
            
            if (row == null || row == 0) {
                (items as any)[this.getIdProperty()] = this.getNextId();
                row = items[id];
            }
            row = 1;
            var item = this.itemAt(row) as TItem;
            if (!AS.IsNullObject(items)) {
                RecordID = items[this.getIdProperty()];
            } else {
                RecordID = row;
            }
            ...
            if (target.hasClass('inline-action') || target.hasClass('inline-actions delete-row') || target.hasClass('delete-row')) {
                //e.preventDefault();
        ...
        //        } else {
        //            Q.confirm(AS.text('Db.Administration.Translation.DeleteWarning', 'Delete record?'), () => {
        //                let o = this as any;
        ...
        protected onViewProcessData(response: Serenity.ListResponse<TItem>): Serenity.ListResponse<TItem> {
            ...
            if (this.getASGridOptions().ShowRowNumberColumn == true) {
        ...
        protected onItemsChanged(value: TItem) {
