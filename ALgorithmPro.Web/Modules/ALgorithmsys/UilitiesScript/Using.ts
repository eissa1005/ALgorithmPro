    ...
    if (window['Vue']) {
        ...
        window['Vue'].filter('formatDateReadable', function (value) {
            if (value) {
                ...
                return date.getDate() + ' ' + AS.Months[date.getMonth()].substr(0, 3) + ' ' + date.getFullYear();
            }
        });
        ...
        window['Vue'].filter('monthOnly', function (value) {
            if (value) {
                let date = Q.parseISODateTime(value);
                return AS.Months[date.getMonth()];
            }
        });
        window['Vue'].filter('monthOnly3', function (value) {
            if (value) {
                let date = Q.parseISODateTime(value);
                return AS.Months[date.getMonth()].substr(0, 3);
            }
        });
        ...
        window['Vue'].filter('formatDateTimeReadable', function (value) {
            if (value) {
                let date = Q.parseISODateTime(value);
                return date.getDate() + ' ' + AS.Months[date.getMonth()] + ' ' + date.getFullYear()
...
function usingJsonDiffPatch() {
    if (window['jsondiffpatch']) {
        ...
        $("<link/>").attr("type", "text/css").attr("id", "JsonDiffPatch").attr("rel", "stylesheet")
            .attr("href", Q.resolveUrl("~/Scripts/jsondiffpatch/formatters-styles/html.css"))
            .appendTo(document.head);


        loadScript(Q.resolveUrl("~/Scripts/jsondiffpatch/jsondiffpatch.min.js"))
        loadScript(Q.resolveUrl("~/Scripts/jsondiffpatch/jsondiffpatch-formatters.min.js"))
    }
}
...
function usingSlickGridEditors() {
    if (window['Slick'] && window['Slick']['Editors'] && window['Slick']['Editors']['Text']) {
        ...
        loadScript(Q.resolveUrl("~/Scripts/Editors/slick.editors.js"))
    }
}
...
function usingSlickAutoColumnSize() {
    if (window['Slick'] && window['Slick']['AutoColumnSize']) {
        ...
        loadScript(Q.resolveUrl("~/Scripts/CustomSlickGridPlugin/slick.autocolumnsize.js"))
    }
}
...
function usingSlickHeaderFilters() {
    if (window['Slick'] && window['Slick']['HeaderFilters']) {
        ...
        .attr("rel", "stylesheet")
            .attr("href", Q.resolveUrl("~/Scripts/CustomSlickGridPlugin/slick-headerfilters.css"))
        .appendTo(document.head);

        loadScript(Q.resolveUrl("~/Scripts/CustomSlickGridPlugin/slick.headerfilters.js"));
    }
}