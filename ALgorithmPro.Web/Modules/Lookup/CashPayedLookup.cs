...
{
    [LookupScript]
    public class CashPayedLookup : RowLookupScript<ASTRTYRow>
    {
        public CashPayedLookup(ISqlConnections sqlConnections) : base(sqlConnections)
        {
        ...
        {
            var CashPayed = (int)TRTYType.CashPayed;
           var fld = ASTRTYRow.Fields;
            query.Distinct(true)
                .Select(fld.TR_TY)
                .Select(fld.Dscr_AR)
                .Where(
                    new Criteria(fld.TR_TY) == CashPayed &
