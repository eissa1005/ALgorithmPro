...
{
    [LookupScript]
    public class CashReceiveLookup : RowLookupScript<ASTRTYRow>
    {
        public CashReceiveLookup(ISqlConnections sqlConnections) : base(sqlConnections)
        {
        ...
        {
            var CashReceive = (int)TRTYType.CashReceive;
           var fld = ASTRTYRow.Fields;
            query.Distinct(true)
                .Select(fld.TR_TY)
                .Select(fld.Dscr_AR)
                .Where(
                    new Criteria(fld.TR_TY) == CashReceive &
